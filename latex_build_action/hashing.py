...
from dirhash import dirhash, included_paths  # type: ignore[import-untyped]
...
def hash_directory(path: Path, ignore: Iterable[str] = DEFAULT_IGNORE_PATTERNS) -> str:
    """Hash the given directory while using the provided list of patterns for exclusions.
    ...
    log.debug(
        "Dirhash (%s) for path %s is %s (ignored patterns: %s)",
        HASH_ALGORITHM,
        str(path),
        hash_str,
        str(ignore),
    ...
    log.debug(
        "Included files:\n%s",
        "\n".join(included_paths(path, HASH_ALGORITHM, ignore=ignore)),
    )
    return hash_str


def read_cached_dirhash(basepath: Path) -> str | None:
    """Read the cached hash from the .checksum file at the given basepath and returns it.
    ...
    if dirhash_file.is_file():
        with dirhash_file.open(encoding="UTF-8") as f:
            existing_hash = f.read().strip()
            log.debug(
                "Stored .checksum (%s) for path %s is %s",
                HASH_ALGORITHM,
                str(basepath),
                existing_hash,
            )
    ...
    log.debug(
        "No stored dirhash (%s) for path %s as no .checksum file was found",
        HASH_ALGORITHM,
        str(basepath),
    )
    return None
...
def cache_dirhash(basepath: Path, h: str) -> None:
    """Cache the given hash in a .checksum file at the given basepath.
    ...
    dirhash_file = basepath.joinpath(".checksum")
    log.debug(
        "Caching hash for path %s in file %s. Cached hash = %s",
        str(basepath),
        str(dirhash_file),
        str(h),
    )
    with dirhash_file.open("w", encoding="UTF-8") as f:
        f.write(h)


def check_dirhash(basepath: Path, ignore: Iterable[str] = DEFAULT_IGNORE_PATTERNS) -> tuple[bool, str]:
    """Compare the hash generated from the given directory with any already saved hash in the same directory.
    ...
    log.debug(
        "Checking cached hash for path %s. Cached hash = %s, new hash = %s",
        str(basepath),
        str(saved_hash),
        str(new_hash),
    )
...
def check_and_update_hash[R](
    basepath: Path,
    on_mismatch: Callable[[], tuple[bool, R]],
    ignore: Iterable[str] = DEFAULT_IGNORE_PATTERNS,
) -> R | None:
    """Check whether the target directory has a matching cached hash (in a .checksum file).

    If the hashes differ, it executes the given callback
    ...
    if hashes_match:
        log.debug(
            "Hashes for path %s matched (ignored patterns: %s)",
            str(basepath),
            str(ignore),
        )
        return None
    ...
    log.debug(
        "Hash mismatch for path %s matched (ignored patterns: %s)",
        str(basepath),
        str(ignore),
    )
    ...
    if cache:
        log.debug(
            "Function after hash mismatch in path %s resulted in new hash %s",
            str(basepath),
            str(new_hash),
        )
        cache_dirhash(basepath, new_hash)
    log.debug("Result after hash mismach in path %s is %s", str(basepath), str(result))
    return result
